...
export default class extends Component {
    ...
    render() {
        ...
        let html = this.props.data.map((obj, index) =>
            <i className='e-option' key={obj.id} data-index={index} onClick={this.handleClick}>{obj.name}</i>
        );
        return (
            <Dish title='编辑衣物信息' onClose={this.props.onClose}>
                <div className='clothes-cate-top'>
                    <span>衣物类别</span>
                    <button type='button' className='e-btn larger' onClick={this.props.onClick}>临时衣物</button>
                </div>

...
export default class extends Component {   
    ...
    doCompensate() {
        ...
        if (!this.state.cardPay && this.state.payStatus =='1' && this.state.returnCash.length == 0) return tool.ui.error({ title: '提示', msg: '现金退款不能为空！', button: ['确定'], callback: (close, event) => { close(); } });
        ...
        if (this.state.cardPay && this.state.payStatus == '1' && this.state.returnCash.length == 0 && this.state.returnCard.length == 0 ) return tool.ui.error({ title: '提示', msg: '请填写至少一种退款金额！', button: ['确定'], callback: (close, event) => { close(); } });

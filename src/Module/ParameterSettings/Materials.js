...
export default class extends Component {   
    ...
    addColorYES (){
        if ('' == this.state.colorname) return tool.ui.error({msg:'请输入材料名称！',callback:close => close()});
    ...
    render() {      
        ...
        return ( 
            <div className='e-container'>
                <button className="e-btn larger price-setting-btn-area" onClick={e => this.setState({show:true,colorname:''})}>新 增 材 料</button>
                <div className='price-setting-table-area'>
                    <Table>
                        <thead>
                            <tr><th>ID</th><th>材料名称</th><th>操作</th></tr>
                        </thead>
                        <tbody> 
                            {colorlist}                         
                        </tbody>
                ...
                <Page className='price-setting-page-area' current={this.state.page} total={this.state.count} fetch={this.limit} callback={page => this.query(page)}/>
                {
                    this.state.show
                    &&
                    <Dish title='新增材料' onClose={() => this.setState({show:false})} width="389" height='194'>
                ...
                }
                {
                    this.state.show1
                    &&
                    <Dish title='修改材料' onClose={() => this.setState({show1:false})} width="389" height='194'>
                        <div className="addbrand-div">
                            <div className="brand-name">材料名称</div>
                            <input  type="text" className="brand-text" value={this.state.colorname} onChange={e => this.setState({colorname:e.target.value})}/>
                        </div>
